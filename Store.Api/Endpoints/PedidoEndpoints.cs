using MediatR;
using Create = Store.Application.UseCases.Pedido.Create;
using Delete = Store.Application.UseCases.Pedido.Delete;
using GetAll = Store.Application.UseCases.Pedido.GetAll;
using GetById = Store.Application.UseCases.Pedido.GetById;
using Update = Store.Application.UseCases.Pedido.Update;
    ...
    public static void MapPedidoEndpoints (this IEndpointRouteBuilder app)
    {
        ...
        group.MapPost("", async (Create.Command command, ISender sender, CancellationToken ct) =>
        {
            ...
            return result.IsSuccess
                ? Results.Created($"/pedidos/{result.Value.Id}", result.Value)
